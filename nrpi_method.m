...
function [v, iteration] = nrpi_method(load_data, line_data, slack_bus_voltage, tolerance, max_iter, runs, v_base, s_base)
   ...
   prompt = {'Ensure the argument data i.e, load and line data are in this format:'
    ...
    Jacobian_time_analysis = zeros(1, runs);
    Jacobian_time = zeros(runs, max_iter);
    ...
    for run = 1:runs
        ...
        Jacobian_start_time_analysis = tic;

        % Build the Jacobian matrix using the help function script
        Jacobian_analysis = build_Jacobian(v, Y_bus, num_buses); %#ok<NASGU>
        
        Jacobian_time_analysis(run) = toc(Jacobian_start_time_analysis);
        ...
        for iteration = 1:max_iter % Iterate up to max_iter times to perform the BFS algorithm.
            ...
            Jacobian_time(run, iteration) = toc(Jacobian_start_time);
    ...
    fprintf('\nThe Jacobian was computed in %f secs\n', mean(Jacobian_time_analysis));

    % compute average time for Jacobian in each iteration per run
    avg_Jacobian_time = mean(Jacobian_time, 1);
    average_Jacobian_time = avg_Jacobian_time(1:iteration);
    
    % Calculate and print the computation time for Jacobian
    fprintf('\nThe Jacobian was computed in secs\n')
    disp(average_Jacobian_time);
    ...
    % grid on;
    filename1 = sprintf('nrpi_vol_profile_%d_bus.fig', num_buses);
    filename2 = sprintf('nrpi_vol_profile_%d_bus.png', num_buses);
    savefig(fig, fullfile(pwd, 'Figures', filename1));
    exportgraphics(gcf, fullfile(pwd, 'Figures', filename2), 'Resolution', 300);
    close(fig);
    ...
    % grid on;
    filename1 = sprintf('nrpi_maxE_Comp_%d_bus.fig', num_buses);
    filename2 = sprintf('nrpi_maxE_Comp_%d_bus.png', num_buses);
    savefig(fig, fullfile(pwd, 'Figures', filename1));
    exportgraphics(gcf, fullfile(pwd, 'Figures', filename2), 'Resolution', 300);
    close(fig);
    
    % Write results to a text file with UTF-8 ecoding
    fid_txt = fopen(sprintf('Result_files/nrpi_bus_voltage_%d_bus.txt', num_buses), 'w', 'n', 'UTF-8');
    fprintf(fid_txt, 'Converged in %d iterations.n\n\n', iteration);
    ...
    writetable(T, sprintf('Result_files/nrpi_bus_voltage_%d_bus.csv', num_buses));
